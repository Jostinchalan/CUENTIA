...
def generar_pdf_cuento(cuento):
    ...
    # Metadatos del cuento
    meta_info = f"""
    <b>Personaje Principal:</b> {cuento.personaje_principal}<br/>
    <b>Tema:</b> {cuento.get_tema_display()}<br/>
    <b>Edad Recomendada:</b> {cuento.edad}<br/>
    ...
    if cuento.imagen_url and not cuento.imagen_url.startswith('/static'):
        try:
            # Descargar imagen
    ...
    for i, paragrafo in enumerate(paragrafos):
        if paragrafo.strip():
            ...
            if i == 0:
                primera_letra = paragrafo[0].upper()
                resto_texto = paragrafo[1:]
    ...
    • Longitud: {cuento.get_longitud_display()}<br/>
    • Tiempo estimado de lectura: {int(cuento.tiempo_lectura_estimado // 60)} minutos<br/>
    • Veces leído: {cuento.veces_leido}<br/>
